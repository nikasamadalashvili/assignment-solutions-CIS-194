...
import Prelude (Show(..), Eq(..), ($), (.), flip, Num (abs))
...
disjunctive_syllogism pOrQ notP = case pOrQ of
    Left p -> absurd $ notP p
    Right q -> q

-- Exercise 2 -----------------------------------------

composition :: (p -> q) \/ (p -> r) -> p -> q \/ r
composition pqOrPr p = case pqOrPr of
    Left pq -> Left $ pq p
    Right pr -> Right $ pr p
...
transposition = Conj dir1 dir2
    where
        dir1 = modus_tollens
        dir2 nQP p = n $ modus_tollens nQP (\nP -> nP p)
            where
                (Conj _ n) = double_negation
...
de_morgan = Conj dir1 dir2
    where
        dir1 nPOrQ = Conj (nPOrQ . Left) (nPOrQ . Right)
        dir2 (Conj nP nQ) = test
            where
                test pOrQ = case pOrQ of
                    Left p -> nP p
                    Right q -> nQ q
